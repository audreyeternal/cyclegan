...
from cyclegan.tester import Tester
...
class CycleGANTester(Tester):
    def __init__(self, **params):
        super(CycleGANTester, self).__init__(**params)
    
    def get_image(self, data):
        data_opts = self.opts.dataset
        dataset_type = data_opts["dataset_type"]
        if dataset_type == "nature_image":
    ...
    def get_metric(self, metric):
        def measure(x, y):
            ...
            return metric(x, y, data_range=1.0)#2021-5-15 真正计算ssim,psnr的地方
        return measure
    ...
    def get_visuals(self, n=8):
        ...
        return self.model._get_visuals(lookup, n, visual_func, False)#2021-5-15 在base.py中，normalize=false return {tags: self._make_visuals(images, len(pairs), normalize)}
#_make_visual 返回图片npy格式
...
if __name__ == "__main__":
    tester = CycleGANTester(
        name="cyclegan", model_class=CycleGANTest,
