...
from cyclegan.datasets import get_dataset
from cyclegan.models import CycleGANTrain
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train an cyclegan network")
    parser.add_argument("run_name", help="name of the run")
    parser.add_argument("--default_config", default="config/cyclegan.yaml", help="default configs")
    parser.add_argument("--run_config", default="runs/cyclegan.yaml", help="run configs")
    args = parser.parse_args()

    # Get CycleGAN options
    ...
    # Get model
    model = CycleGANTrain(opts['learn'], opts['loss'], **opts['model'])
    ...
    for epoch in range(start_epoch, opts['num_epochs']):
        for data in logger(train_loader):
            print(data[1].shape,data[1].dtype,len(data[0]))
            model.optimize(*data)
        model.update_lr()
