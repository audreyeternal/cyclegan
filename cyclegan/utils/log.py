...
class Logger(object):
    ...
    def add_metric_log(self, pair_fcn, metrics_fcns, metrics_freq=1):
        self.pair_fcn = pair_fcn#2021-5-15 test.py中的get_pairs函数,返回("before", (img_low, img_high)), ("after", (pred_lh, img_high))], self.model.name
        self.metrics_cnt = 0
        self.metrics = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))#2021-5-15 有点看不懂
        self.metrics_fcns = metrics_fcns#(("ssim", self.get_metric(ssim)), ("psnr", self.get_metric(psnr)))
        self.metrics_freq = metrics_freq

    def add_iter_visual_log(self, iter_visual_fcn, iter_visual_freq, name=""):#2021-5-15 iter_visual_fcn:get_visuals
    ...
    def __call__(self, iterable):
        ...
        for it, obj in enumerate(progress):
            ...
            if hasattr(self, 'iter_visual_fcn') and it % self.iter_visual_freq == 0:
                for k, v in self.iter_visual_fcn().items():#get_visuals是iter_visual_fcn
                    ...
                    visual_file = path.join(iter_visual_dir,
                        "epoch{}_iter{}_{}.png".format(self.epoch, it, k))#我们经常看到的图片在这里
                    Image.fromarray(v).convert('RGB').save(visual_file)
 
            if hasattr(self, 'pair_fcn') and it % self.metrics_freq == self.metrics_freq - 1:#2021-5-15 类似8/3 = 2...2
                pairs, name = self.pair_fcn()
                for i in range(len(pairs[0][1][0])):
                    n = len(self.metrics) - ('mean' in self.metrics)#（ in ...):1 or 0
                    for j, pair in pairs:
                ...
                metric_file = path.join(self.log_dir, "metrics_{}.yaml".format(self.epoch))#2021-5-15 只保存最后一次
