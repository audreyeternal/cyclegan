...
class Base(nn.Module):
    ...
    def _get_visuals(self, lookup, n, func=None, normalize=True):
        ...
        pairs = [(t, func(getattr(self, k)[:n])) for t, k in lookup if hasattr(self, k)]#2021-5-15 ("l", "img_low");fun是visual_fcn，调窗；n应该是每次显示多少，默认是8
        tags, images = zip(*pairs)#2021-5-15 解压
...
class BaseTrain(Base):
    ...
    def _get_trainer(self, model_dict, model_type, opt_type="adam"):
        model = model_dict[model_type]()#2021-4-24 model_dict 在cyclegantrain __init__中，分为 cyclegan 和 nplayerdis.
        optimizer, scheduler = self._setup_learn(model.parameters(), opt_type)

        def _clear(self, backprop=True): #2021-4-21 没有返回值会默认返回None
            """ Clear gradient
            """
        ...
        class CriterionDict(object):
            ...
            def __setitem__(self, key, item):#2021-4-21 待会查一下
        ...
        model.__dict__['_update'] = types.MethodType(_update, model) #2021-4-21 动态添加实例方法
    ...
    def _get_criterion(self, loss_dict, loss_wgts, tag=""):#2021-4-21 loss_dict 在CycleGANTrain __init__中定义,有l1,gl,gh
    ...
    def update_lr(self):
        for k in dir(self):
            if k.startswith('_'): continue #2021-4-20 将私有变量跳过？
            v = getattr(self, k)
