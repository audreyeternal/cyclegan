...
from ..networks import CycleGAN, NLayerDiscriminator, add_gan_loss
...
class CycleGANTrain(BaseTrain):
    def __init__(self, learn_opts, loss_opts, g_type, d_type, **model_opts):
        super(CycleGANTrain, self).__init__(learn_opts, loss_opts)
        g_opts, d_opts = model_opts[g_type], model_opts[d_type]

        model_dict = dict(
              cyclegan = lambda: CycleGAN(**g_opts),
            nlayer = lambda: NLayerDiscriminator(**d_opts))

        self.model_g = self._get_trainer(model_dict, g_type) # CycleGAN generators #2021-4-21 _get_trainer在basetrain中,给model加了很多参数
        ...
        loss_dict = dict(
               l1 = nn.L1Loss,
               gl = (self.model_dl.get_g_loss, self.model_dl.get_d_loss), # GAN loss for low quality image.#2021-4-21 为什么没有传参数?
        ...
        self.model_g._criterion["ll"] = self._get_criterion(loss_dict, self.wgts["ll"], "ll_")#2021-4-21 _criterion:在BaseTrain中的_get_train()中增加的属性,wgts应该在config/cyclegan中定义。
        self.model_g._criterion["lh"] = self._get_criterion(loss_dict, self.wgts["lh"], "lh_")#_get_criterion函数返回一个criterion类
    ...
    def optimize(self, img_low, img_high):
        ...
        if self._nonzero_weight("gl", "lh", "ll"):
            ...
            self.pred_ll, self.pred_lh = self.model_g.forward1(self.img_low) #2021-4-21 model_g model_gl model_gh 是主要的,pred_ll等是由他们生成
            self.model_g._criterion["gl"](self.pred_lh, self.img_high)#2021-4-21 因为_criterion返回一个criterion类，所以可以继续在后面加() 
        ...
        self.model_g._update() #2021-4-21 先训练d后训练g
